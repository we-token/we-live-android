...
public class PullToRefreshAttacher {
    ...
    /**
     * PullToRefreshAttacher事件回调，将所有attacher保存到activity中
     */
    public static interface OnPullToRefreshAttacherEventListener {
        /**
         * 添加attacher到activity
        ...
        /**
         * 获取activity的attacherSet
         * @return Set<PullToRefreshAttacher>
         */
    ...
    private final long mRefreshMinimizeInterval;
    private final long mRefreshTimeoutDuration;
    private long mLastRefreshTime;
    private boolean mIsDestroyed = false;

    private final int[] mViewLocationResult = new int[2];
    private final Rect mRect = new Rect();
    private boolean mHeaderViewAlreadyAddToActivity = false;
    ...
    protected PullToRefreshAttacher(Activity activity, Options options) {
        ...
        mRefreshMinimizeInterval = options.refreshMinimizeInterval;
        mRefreshTimeoutDuration = options.refreshTimeoutDuration;
        ...
        mHeaderView = decorView.findViewById(options.headerId);
        ...
        /*decorView.post(new Runnable() {
            ...
            public void run() {
                if (decorView.getWindowToken() != null) {
                    Log.d("xh", "post addHeaderViewToActivity " + PullToRefreshAttacher.this);
        ...
        if (refreshing) {
            ...
            if (canRefresh(false)) {
                ...
                if (mOnRefreshListener != null) {
                    mOnRefreshListener.onRefreshStarted(null);
                }
            } else {
                reset(false);
            }
    ...
    void hideHeaderView() {
        if (mHeaderTransformer != null && mHeaderTransformer.hideHeaderView()) {
            if (mHeaderViewListener != null) {
                mHeaderViewListener.onStateChanged(mHeaderView,
    ...
    private void reset(boolean fromTouch) {
        getHeaderView().removeCallbacks(mResetImmediatelyRunnable);
        if (mIsRefreshing) {
            mIsStopping = true;
            long delay = mRefreshMinimizeInterval - (System.currentTimeMillis() - mLastRefreshTime);
            if (delay > 0) {
                getHeaderView().postDelayed(mResetImmediatelyRunnable, delay);
            } else {
                getHeaderView().post(mResetImmediatelyRunnable);
            }
        } else {
            resetImmediately();
        }
    ...
    private void resetImmediately() {
        ...
        if (mRefreshMinimize && getHeaderView() != null) {
    ...
    private void startRefresh(View view, boolean fromTouch) {
        if (mActivity instanceof OnPullToRefreshAttacherEventListener) {
            Set<PullToRefreshAttacher> attacherSet = ((OnPullToRefreshAttacherEventListener) mActivity).getAttacherSet();
            for (PullToRefreshAttacher attacher : attacherSet) {
                if (attacher != this && attacher.isRefreshing()) {
                    attacher.resetImmediately();
                }
            }
        }
        ...
        getHeaderView().postDelayed(mResetImmediatelyRunnable, mRefreshTimeoutDuration);
    ...
    public boolean isHeaderViewAlreadyAddToActivity() {
        return mHeaderViewAlreadyAddToActivity;
    }
    ...
    protected void addHeaderViewToActivity(View headerView) {
        ...
        mHeaderViewAlreadyAddToActivity = true;
    }
    ...
    protected void removeHeaderViewFromActivity(View headerView) {
        if (headerView.getWindowToken() != null) {
            ...
            mHeaderViewAlreadyAddToActivity = false;
        }
    }

    private final Runnable mResetImmediatelyRunnable = new Runnable() {
        @Override
        public void run() {
            resetImmediately();
        }
